...
template MonthRegex(msg_bytes) {
	...
	for (var i = 0; i <= num_bytes; i++) {
		is_accepted.in[i] <== states[i][16];
	}
	out <== is_accepted.out;
	signal is_consecutive[msg_bytes+1][3];
	is_consecutive[msg_bytes][2] <== 0;
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][16] * (1 - is_consecutive[msg_bytes-i][2]) + is_consecutive[msg_bytes-i][2];
		is_consecutive[msg_bytes-1-i][1] <== state_changed[msg_bytes-i].out * is_consecutive[msg_bytes-1-i][0];
		is_consecutive[msg_bytes-1-i][2] <== ORAnd()([(1 - from_zero_enabled[msg_bytes-i+1]), states[num_bytes-i][16], is_consecutive[msg_bytes-1-i][1]]);
	}
	// substrings calculated: [{(12, 14), (14, 15), (15, 16)}]
	signal prev_states0[3][msg_bytes];
	signal is_substr0[msg_bytes];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		 // the 0-th substring transitions: [(12, 14), (14, 15), (15, 16)]
		...
		is_substr0[i] <== MultiOR(3)([prev_states0[0][i] * states[i+2][14], prev_states0[1][i] * states[i+2][15], prev_states0[2][i] * states[i+2][16]]);
