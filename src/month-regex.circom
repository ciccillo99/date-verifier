...
template MonthRegex(msg_bytes) {
	...
	// substrings calculated: [{(13, 15), (15, 16), (16, 17)}]
	signal prev_states0[3][msg_bytes];
	signal is_substr0[msg_bytes];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		 // the 0-th substring transitions: [(13, 15), (15, 16), (16, 17)]
		prev_states0[0][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][13];
		prev_states0[1][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][15];
		prev_states0[2][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][16];
		is_substr0[i] <== MultiOR(3)([prev_states0[0][i] * states[i+2][15], prev_states0[1][i] * states[i+2][16], prev_states0[2][i] * states[i+2][17]]);
